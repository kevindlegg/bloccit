...
RSpec.describe SessionsController, type: :controller do
  ...
  describe "POST sessions" do
    it "returns http success" do
      post :create, params: { sesssion: { email: my_user.email } }
      expect(response).to have_http_status(:success)
    end
    ...
    it "flashes #error with bad email address" do
      post :create, params: { sesssion: { email: "does not exist" } }
    ...
    it "redirects to the root view" do
      post :create, params: { sesssion: { email: my_user.email, password: my_user.password } }
